...
**Current status:** Phase 1 complete (WP1 + WP2 + WP3); Technical Report v1.0 candidate frozen pending Atlas-integrity reader pass + Zenodo DOI. **Phase 2 Projektantrag draft v0.2** ([docs/projektantrag_p2.md](docs/projektantrag_p2.md)) is in pre-freeze state — pending Integrator confirmation of Ledger entry CL-2026-006 and Handbook §7 anchor before v1.0 freeze and any WP-α₀ work begins.

**Phase 1 headline:** **DG-2 NOT MET (negative result, recorded as anticipated)**: `admec_full` beats the best non-ADMEC baseline on MSE in one of eight scenarios (S2, fully connected, low delay). The delay-restricted local consensus cannot close the gap to centralised exclusion methods on sparse-with-delay topologies; the constraint layer beats `admec_delay` everywhere but cannot rescue DG-2. WP3 ablations characterised this failure as topology-dominated (the *N*/*k*_eff pooling-reference baseline) rather than a tuning problem.
...
1. **[Technical Report v1.0 candidate](docs/manuscript.md)** — the full WP2 + WP3 characterisation: campaign results, all five ablations, the topology-pooling-reference figure, mechanism, operational recommendations, and follow-up redesign directions. Reads in ~15 min. Citable through release tag `v1.0-tech-report` (pending Atlas-integrity reader pass + Zenodo DOI).
...
5. **[Phase-1 Project Proposal](docs/projektantrag.md)** — pre-registered objectives, work packages, decision gates (Phase 1, frozen v0.5.3).
6. **[Phase-2 Project Proposal](docs/projektantrag_p2.md)** — Phase 2 Projektantrag draft v0.2: two consensus-rule redesigns (α-rule, λ-rule) on the topological pooling-reference baseline, with new metrology-anchored scenarios S9 (fibre chain) and S10 (hub-and-spoke). **Pending v1.0 freeze**; no WP work has started.
7. **[Latest logbook entry](logbook/012_2026-05-05_wp3-ablation-lagged-classification.md)** — WP3 ablation 5 (lagged classification, closes the Phase-1 systematic sweep).
8. **[Source code](https://github.com/threehouse-plus-ec/admec-clock-consensus/tree/main/src)** — the implementation.
...
|----------|-------------|
| [Technical Report v1.0 candidate](docs/manuscript.md) | The full Phase-1 WP2 + WP3 characterisation, prepared as a citable technical report rather than a journal manuscript. Self-contained ~15 min read; tables reproducible from the canonical archives. Citable via release tag + Zenodo DOI once the Atlas-integrity reader pass is complete. |
| [Phase-1 Project Proposal](docs/projektantrag.md) | Internal project proposal (DFG Sachbeihilfe structure, v0.5.3 frozen). Defines Phase-1 objectives, work packages (WP1–WP3), scenarios (S1–S8), estimators, decision gates DG-1/2/2b/3, and failure conditions. |
| [Phase-2 Project Proposal](docs/projektantrag_p2.md) | Phase-2 Projektantrag draft v0.2 (Coastline document under Open-Science Harbour template v2.0). Two consensus-rule redesigns (WP-γ α-rule, WP-δ λ-rule) tested against the *N*/*k*_eff pooling-reference baseline; six decision gates DG-α₀ through DG-ε; new scenarios S9, S10. **Pending v1.0 freeze** (Ledger CL-2026-006 + Handbook §7 anchor). |
| [Handbook](docs/handbook.md) | Reusable methodology reference. Procedures live here; outcomes do not. **§7 Statistical Methodology** (Phase-2 prerequisite) is a v1.0-freeze stub — content lands when WP-α₀ opens. |
| [Claim Analysis Ledger](docs/ledger.md) | Registry of claim-shaped outcomes across phases. Phase-2 entry CL-2026-006 (proposed) is staged here for Integrator confirmation at v1.0 freeze. |
